...
| `setup_sector1_content.py` | Builds the Sector 1 content in the editor: creates and maps `IA_Interact` (E) and `IA_Rewind` (hold R) and binds them on the player Blueprint, creates the script assets under `/Game/Scripts` (the airlock door, the corridor lights), and rebuilds `/Game/Sectors/L_Sector1_Airlock` (corridor, door, two terminals, wall display, lights, exposure). Re-runnable; run through `ue_remote_python.py`. |
