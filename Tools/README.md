...
| `setup_phase2_content.py` | Builds the Phase 2 content in the editor: binds `IA_Interact` on the player Blueprint, creates `/Game/Scripts/DA_Airlock_InnerDoor`, and rebuilds `/Game/Sectors/L_Sector1_Airlock` (corridor, door, terminal, wall display, lights, exposure). Re-runnable; run through `ue_remote_python.py`. |
