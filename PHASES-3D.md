...
| 5 — Power | done 11 Sep 2026: one bus per sector on the station subsystem (budget 40, reserve 24 in Sector 1), per-script `RunCost`/`PredictedRunCost` with the committed-reading discount, lights dim in stages (nominal, low, critical, out) with amber emergency lighting and a camera torch at out, `AGitsGenerator` draws the reserve cell, `AGitsPowerGauge` on the wall, notes charge the bus, runs refused when it cannot pay, validator rules on every script at level start (`GitsTags::ValidatePower`, tested). Screenshots in `docs/screenshots/phase-5` |
| 6 — Blender kit and Sector 1 blockout | done 11 Sep 2026: fifteen kit pieces built by `Tools/build_kit.py` in Blender headless on one painted trim sheet (`Tools/make_trim_sheet.py`), bevelled, 84 collision boxes through the sidecar; Sector 1 blocked out from the kit (entry airlock, main corridor, generator room, cold store, corner, airlock to Sector 2, sealed door beyond) with the rig, emergency lights, cold fills and dressing; walked end to end in PIE at 60 fps (average 59.6 to 59.9 over four windows; the only frames under 60 were the ones running console commands). Eight views in `docs/screenshots/phase-6` |
| 7 — Sector 1 complete: the vertical slice | built 11 Sep 2026: eight systems re-authored from Act 1 with tags, reveals, hints, logs and unlocks; goals of three kinds incl. the Make task with tests; panels bound per system; vent and shift clock; the airlock opens on the sector. Played end to end by automation in PIE (`docs/screenshots/phase-7`). **Open:** the acceptance is a human playtest (a non-programmer, under 45 minutes, a laugh, the airlock) and a recording; that is the developer's to run |
| 8 — Telemetry and instruments | built 11 Sep 2026: consent-gated event log (`UGitsTelemetrySubsystem`), the reference's eighteen event types wired through VANT, the station and the terminal, the four instruments ported with the tracing items verified against the interpreter, two study terminals in Sector 1, export in the reference's bundle format read by its `analyse.ts` without change (`docs/screenshots/phase-8`) |
| 9 — Sectors 2 to 4 | built 11 Sep 2026: three maps (Greenhouse, Logistics, Reactor), 22 systems re-authored from Acts 2 to 4 with fittings (heaters, sprinklers, conveyors, lamps, the mast), sector gates between maps with progress carried in the game instance, the rooms view for Sector 4, Ilse's story resolved at the west door; `check_curriculum.py` shows every concept in 2+ systems and every misconception in 2+ (`docs/screenshots/phase-9`). Sector 3's Make is the reference's depot tally; the drone route comes with Phase 10's drones |
